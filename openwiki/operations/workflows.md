...
description: "How to run, ship and change retail-portfolio: the Docker Compose-only dev stack and root .env contract, the agent-test harness gates, git-worktree isolation for parallel agents, migration rules, CLI commands, background jobs and the Huey dashboard, the three CI jobs, the deployment surface, the OpenSpec spec-driven workflow, and the scheduled OpenWiki refresh."
tags: [development-workflow, ci, docker-compose, migrations, openspec, background-jobs, deployment, agent-harness]
sources:
...
migrations drift: it drops every table, clears `alembic_version`, runs `alembic upgrade head`,
then asserts an autogenerated revision contains no `op.<name>(` calls.
...
WebSocket at `/worker/api/updates` that authenticates via a signed ticket or the
`auth_token` cookie / `sec-websocket-protocol` header, closing with code `1008` when neither
validates. `init_worker_dashboard` creates the task table, binds signal handlers
(`bind_signals=True`) and starts a Redis pub/sub listener; the lifespan closes it on shutdown.
In dev the `worker` service sets `HUEY_DASHBOARD_WORKER: 1` so the consumer process emits the
signal events the dashboard subscribes to. The Redis transport underneath both the queue and
this dashboard, and the WebSocket fan-out the tasks drive, are owned by
