...
- If you stage the plan as a local file first, write it to `.agent/plans/<ticket-id>-plan.md`.
- Temp files (e.g. a `--body-file` payload) go to `.agent/scratch/` — never the repo root or `/tmp`. Prefer `gh issue edit <N> --body-file -` (stdin) when you don't need a local copy.
