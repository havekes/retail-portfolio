...
async def test_positions_by_account_empty(auth_client, test_accounts):
    ...
    response = await auth_client.get(f"/api/positions/{account_id}")
...
@pytest.mark.anyio
async def test_positions_by_account_success(auth_client, test_accounts, test_positions):
...
async def test_positions_by_account_not_found(auth_client):
    ...
    response = await auth_client.get(f"/api/positions/{fake_account_id}")
