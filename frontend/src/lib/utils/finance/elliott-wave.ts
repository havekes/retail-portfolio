...
/**
 * Normalizes a SecurityElliottWaves object by ensuring a valid waves array.
 */
export function normalizeSecurityElliottWaves(
	waves: SecurityElliottWaves | null | undefined
): SecurityElliottWaves {
	if (!waves || !Array.isArray(waves.waves)) {
		return { waves: [] };
	}
	return {
...
/**
 * Returns all wave counts for a security from the `waves` collection.
 */
export function getSecurityWaveCounts(
	waves: SecurityElliottWaves | null | undefined
): DegreeWaveCount[] {
	if (!waves || !Array.isArray(waves.waves)) return [];
	return waves.waves;
}

/**
 * Compares two SecurityElliottWaves objects for structural equality across their wave collections.
 */
export function areSecurityElliottWavesEqual(
	...
	if (!a && !b) return true;
	const wavesA = a?.waves || [];
	const wavesB = b?.waves || [];
...
export function getSecurityDegreeWaveCount(
	...
	return (
		securityWaves.waves.find((w) => w.degree === degree && w.type === 'impulse') ??
		securityWaves.waves.find((w) => w.degree === degree) ??
		null
	);
}
