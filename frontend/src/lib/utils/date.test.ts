...
describe('getChartDateWindow', () => {
	it('should return a 180-day window for intraday interval 1h', () => {
		const endDate = new Date(2026, 7, 12);
		const result = getChartDateWindow(endDate, '1h');
		expect(result.to).toBe('2026-08-12');
		expect(result.from).toBe('2026-02-13');
	});

	it('should return a 180-day window for intraday interval 4h', () => {
		const endDate = new Date(2026, 7, 12);
